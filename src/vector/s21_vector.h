 ...
 public:
  ...
  vector(std::initializer_list<value_type> const &items)
      : size_(items.size()),
        ...
        data_(_AllocateData(this->capacity_)) {
    size_type i = 0;
    try {
  ...
  constexpr ~vector() { _DeallocateData(); }
  ...
  constexpr reference front() { return this->data_[0]; }

  constexpr const_reference front() const { return this->data_[0]; }

  // Доступ к последнему элементу
  constexpr reference back() { return this->data_[this->size_ - 1]; }
  ...
  // Возвращает количество элементов, которые могут на данный момент храниться в
  // векторе
  ...
  constexpr iterator insert(const_iterator pos, const_reference value) {
    ...
    for (size_type i = this->size_; i > index_pos; --i) {
      new (this->data_ + i)
          value_type(std::move_if_noexcept(this->data_[i - 1]));
