...
class map {
 public:
  ...
  constexpr iterator begin() noexcept { return tree_.begin(); }

  constexpr const_iterator cbegin() const noexcept { return tree_.cbegin(); }

  // Возвращает итератор на конец словаря
  constexpr iterator end() const noexcept { return tree_.end(); }

  constexpr const_iterator cend() const noexcept { return tree_.cend(); }

  constexpr iterator find(const key_type& key) { return tree_.find(key); }
  ...
  constexpr bool empty() const noexcept { return tree_.empty(); }

  // Возвращает количество элементов словаря
  constexpr size_type size() const noexcept { return tree_.size(); }

  // Возвращает максимально возможное количество элементов словаря
  constexpr size_type max_size() const noexcept { return tree_.max_size(); }
  ...
  constexpr iterator erase(iterator pos) { return tree_.erase(pos); }

  constexpr iterator erase(const_iterator pos) { return tree_.erase(pos); }
  ...
  // Сращивает узлы из другого словаря
  ...
  // Вставляет несколько элементов в словарь
