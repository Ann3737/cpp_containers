...
TEST(S21QueueInsertManyBack, InsertMultipleValues) {
  s21::queue<int> q;
...
TEST(S21QueueInsertManyBack, InsertLvaluesAndRvalues) {
  s21::queue<std::string> q;
  std::string hello = "hello";
  std::string world = "world";

  q.insert_many_back(hello, std::move(world), "test");

  EXPECT_EQ(q.size(), 3);
  EXPECT_EQ(q.front(), "hello");
  EXPECT_EQ(q.back(), "test");

  EXPECT_TRUE(world.empty() || world == "world");
}

TEST(S21QueueInsertManyBack, InsertIntoNonEmptyQueue) {
  s21::queue<int> q = {10, 20};

  q.insert_many_back(30, 40);
...
TEST(S21QueueInsertManyBack, InsertNoArgsDoesNothing) {
