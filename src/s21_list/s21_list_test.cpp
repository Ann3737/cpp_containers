...
TEST(ListTest, InsertManyAtIterator) {
  s21::List<int> list;
  list.push_back(100);
  list.push_back(200);
  list.push_back(300);
  ...
  auto cur = list.begin();
  for (int val : expected) {
    ASSERT_NE(cur, list.end());
    EXPECT_EQ(*cur, val);
    ++cur;
  }

  // Проверим, что returned итератор указывает на 10
  EXPECT_EQ(*inserted, 10);
}

TEST(ListTest, InsertManyEmptyList) {
  s21::List<int> list;
  auto it = list.insert_many(list.begin(), 7, 8, 9);
...
TEST(ListTest, BackConstThrowsOnEmptyList) {
  ...
  // Проверяем, что при попытке получить последний элемент выбрасывается
  // исключение
