...
class List {
 ...
 public:
  ...
  class iterator {
   ...
   public:
    ...
    iterator(const const_iterator& it)
        : current_(it.current_), owner_(const_cast<List<T>*>(it.owner_)) {}
  ...
  class const_iterator {
   ...
   public:
    /*констуктор*/
    const_iterator(Node* node, const List<T>* owner)
        : current_(node), owner_(owner) {}
  ...
  // Рекурсивная версия — вставляет в конец
  template <typename First, typename... Rest>
  void insert_many_back(First&& first, Rest&&... rest) {
    push_back(std::forward<First>(first));  // сначала вставляем первый
    insert_many_back(
        std::forward<Rest>(rest)...);  // потом вызываем себя с оставшимися
  }

  template <typename First>
  iterator insert_many(const_iterator pos, First&& first) {
    return insert(pos, std::forward<First>(first));
  }

  template <typename First, typename... Rest>
  iterator insert_many(const_iterator pos, First&& first, Rest&&... rest) {
    auto it = insert(pos, std::forward<First>(first));
    insert_many(pos, std::forward<Rest>(rest)...);
    return it;
  }
};
};  // namespace s21
